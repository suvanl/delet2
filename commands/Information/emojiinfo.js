...
class EmojiInfo extends Command {
    ...
    async run(message, args, level, settings, texts) { // eslint-disable-line no-unused-vars
        if (!args[0]) return message.channel.send(texts.cmd.noEmoji);
        if (args[0].charCodeAt(0) >= 55296) return message.channel.send(`${texts.cmd.regularEmoji.replace(/{{emoji}}/g, args[0])}\nhttps://twitter.github.io/twemoji/`);

        const match = args[0].match(/<:[a-zA-Z0-9_-]+:(\d{18})>/);
        if (!match || !match[1]) return message.channel.send(texts.cmd.invalidEmoji);
        ...
        if (!emoji) return message.channel.send(texts.cmd.invalidEmoji);
